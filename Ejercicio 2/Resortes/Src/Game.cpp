...
void Game::DrawGame()
{
	...
	circulo.setPosition(circuloControl->GetPosition().x - 5, circuloControl->GetPosition().y - 5);
	wnd->draw(circulo);

	sf::CircleShape circuloIzq(5);
	circuloIzq.setFillColor(sf::Color::Magenta);
	circuloIzq.setPosition(circuloIzqControl->GetPosition().x - 5, circuloIzqControl->GetPosition().y - 5);
	wnd->draw(circuloIzq);
	...
	while (wnd->pollEvent(evt)) // Procesa todos los eventos acumulados
	{
		...
		case Event::MouseButtonPressed: // Si se presiona un botón del ratón
			...
			// Transformamos las coordenadas segun la vista activa
				...
				Vector2f cirpos = wnd->mapPixelToCoords(Vector2i(circuloControl->GetPosition().x, circuloControl->GetPosition().x));
				if (pos == cirpos) {
					circuloControl->SetTransform(b2Vec2(pos.x, pos.y), 0.0f);
				}
...
void Game::InitPhysics()
{
	...
	/*b2Body* circulo = Box2DHelper::CreateCircularDynamicBody(phyWorld, 5, 1.0f, 0.5, 1.1f);
	circulo->SetTransform(b2Vec2(30.0f, 50.0f), 0.0f);*/

	/*b2Body* circuloIzq = Box2DHelper::CreateCircularDynamicBody(phyWorld, 5, 1.0f, 0.5, 1.1f);
	circuloIzq->SetTransform(b2Vec2(60.0f, 50.0f), 0.0f);*/


	circuloControl = Box2DHelper::CreateCircularDynamicBody(phyWorld, 5, 30.0f, 0.5, 1.1f);
	circuloControl->SetTransform(b2Vec2(30.0f, 50.0f), 0.0f);

	circuloIzqControl = Box2DHelper::CreateCircularDynamicBody(phyWorld, 5, 1.0f, 0.5, 1.1f);
	circuloIzqControl->SetTransform(b2Vec2(60.0f, 50.0f), 0.0f);

	Box2DHelper::CreateDistanceJoint(phyWorld, circuloControl, circuloControl->GetWorldCenter(),
		circuloIzqControl, circuloIzqControl->GetWorldCenter(), 15.0f, 100.1f, 1.0f);
